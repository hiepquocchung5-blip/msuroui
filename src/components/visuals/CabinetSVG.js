...
const CabinetSVG = ({ 
    ...
    // --- 3. TOPPER (Data Counter + Header PNG) ---
    const renderTopper = () => (
        <g>
            {/* Physical Unit */}
            <g transform="translate(50, 0)">
                 <rect x="0" y="0" width="140" height="40" fill="#111" stroke="#333" rx="4" />
                 {/* Header Image Slot (16:9 PNG) */}
                 <image 
                    href={getHeaderImg(islandId)} 
                    x="2" y="2" width="136" height="36" 
                    preserveAspectRatio="none"
                    onError={(e) => { e.target.style.display = 'none'; }} // Hide if missing
                 />
                 {/* Glass over image */}
                 <rect x="0" y="0" width="140" height="40" fill="url(#glassGlare)" opacity="0.3" rx="4" pointerEvents="none"/>
            </g>

            {/* LED Stats (Floating above/integrated) */}
            <g transform="translate(10, 5)">
                 <rect x="0" y="0" width="35" height="30" fill="#000" stroke="#222" rx="2" />
                 <text x="17" y="10" textAnchor="middle" fill="#555" fontSize="5" fontWeight="bold">LAPS</text>
                 <text x="17" y="22" textAnchor="middle" fill="red" fontSize="10" fontFamily="monospace" fontWeight="bold" filter="url(#ledGlow)">{stats.laps}</text>
            </g>
            <g transform="translate(195, 5)">
                 <rect x="0" y="0" width="35" height="30" fill="#000" stroke="#222" rx="2" />
                 <text x="17" y="10" textAnchor="middle" fill="#555" fontSize="5" fontWeight="bold">MODE</text>
                 <circle cx="17" cy="20" r="4" fill={isHot ? 'gold' : (isBusy ? 'lime' : '#333')} className={isHot || isBusy ? 'animate-pulse' : ''} filter="url(#ledGlow)" />
            </g>
        </g>
    );
    ...
    // --- 5. BUTTON DECK (Pachislo Hardware) ---
    const renderButtonDeck = () => (
        <g transform="translate(10, 230)">
             ...
             <path d="M-15,50 L235,50 L235,70 L-15,70 Z" fill="#222" />
    ...
    // --- 6. BELLY GLASS (PNG Art + Overlays) ---
    const renderBelly = () => (
        <g transform="translate(30, 310)">
             ...
             <g clipPath="url(#bellyClip)">
                 ...
                 {/* 3. Sticker Overlay (Dynamic Character) */}
                 <g transform="translate(130, 35) scale(0.25)">
                    <CharacterSVG type={charId} stickerMode={true} />
                 </g>
                 
                 {/* 4. Text Overlay */}
                 <text x="20" y="25" fill="#FFF" fontSize="12" fontWeight="black" style={{textShadow:'0 0 5px white', letterSpacing: '1px'}}>
                     SUPER BONUS
                 </text>
                 <text x="20" y="45" fill="gold" fontSize="10" fontFamily="monospace" fontWeight="bold">
                     WIN: {stats.wins}
                 </text>
             </g>
